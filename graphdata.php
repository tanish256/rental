...
function getYearlyBalances($year) {
    ...
    try {
        $trans_query = "SELECT 
                            month, 
                        SUM(CASE WHEN balance_bf > 0 THEN balance_bf ELSE 0 END) AS total_balance_bf,
                        SUM(CASE WHEN balance_due > 0 THEN balance_due ELSE 0 END) AS total_balance_due,
                        SUM(CASE WHEN total_balance > 0 THEN total_balance ELSE 0 END) AS total_balance
                        FROM balances 
